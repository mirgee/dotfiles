...
for i = 1, 9 do
  vim.keymap.set('n', '<leader>h' .. i, function() hui.nav_file(i) end,
    { desc = 'Harpoon: Go to Mark ' .. i })
end

for i = 1, 9 do
  vim.keymap.set('n', '<leader>t' .. i, function() ht.gotoTerminal(i) end,
...
for i = 1, 9 do
  for j = 1, 9 do
    vim.keymap.set('n', '<leader>c' .. i .. j, function()
      ...
      { desc = 'Harpoon: Send Command ' .. i .. ' to Terminal ' .. j })
  end
end
...
vim.api.nvim_create_user_command(
  ...
  { nargs = '*', desc = 'Send command to Harpoon terminal' }
)
